...
class Tree {
  ...
  // targetData -> 부모노드 즉, 내가 어느 노드의 자식으로 insert할건지 모르기에 targetData == parent를 찾아야됩니다.
  // 그리고 그 parent 아래에 insert하고자하는 데이터를 삽입한다.
  ...
  remove(targetData) {
    ...
    while (queue.length) {
      ...
      let targetIdx = -1; // A 노드에 자식 중 [B,C]있을때 B가 몇번째에 있냐를 의미하고있다.
...
tree.insert(null, "A");
tree.insert("A", "B"); // A밑에 자식 B가 있고
tree.insert("A", "C");
tree.insert("B", "D");
tree.insert("B", "E");
tree.insert("B", "F");
tree.insert("C", "G"); // C밑에 자식 G가 있다.
