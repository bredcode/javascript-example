...
class BST {
  ...
  remove(cur, data) {
    ...
    // 지금 바라보고있는 노드의 데이터가 타겟 데이터보다 큰 경우
    // 왼쪽으로 이동
    if (data < cur.data) {
      cur.left = this.remove(cur.left, data);
    }
    // 지금 바라보고있는 노드의 데이터가 타겟 데이터보다 작은 경우
    // 오른쪽으로 이동
    else if (data > cur.data) {
...
bst.search(bst.root, 32);
bst.search(bst.root, 33);
